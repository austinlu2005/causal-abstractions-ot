"""Transport-based alignment and intervention for RAVEL residual-stream sites."""
...
from mcqa_experiment.intervention import run_soft_residual_intervention
...
def load_prepared_alignment_artifacts(
    ...
    *,
    ...
    """Load cached RAVEL signature artifacts when the on-disk spec matches."""
...
def save_prepared_alignment_artifacts(
    ...
    *,
    ...
    """Persist reusable RAVEL signature artifacts for future sweeps and reruns."""
    path = Path(cache_path)
    ...
    torch.save(
        {
            ...
            "saved_with": "ravel_signature_cache_v1",
...
def run_alignment_pipeline(
    *,
    ...
    """Run OT or UOT for one RAVEL target variable."""
    device = torch.device(device)
    if config.selection_verbose:
        print(
            f"[{config.method.upper()}] start variable={fit_bank.target_var} "
            f"source_type={fit_bank.source_type} signature_mode={config.signature_mode} "
            f"candidate_sites={len(sites)} epsilon={float(config.epsilon):g}"
        )
    ...
    if config.method == "ot":
        ...
        raise ValueError(f"Unsupported RAVEL transport method {config.method}")
    ...
    if config.selection_verbose:
        print(
            ...
            f"source_type={fit_bank.source_type} top_k={top_k} "
            f"lambda={strength:g} exact_acc={float(holdout_result['exact_acc']):.4f}"
        )
    return {
        "target_var": fit_bank.target_var,
        "source_type": fit_bank.source_type,
        "signature_mode": config.signature_mode,
