"""Summary helpers for RAVEL comparison runs."""
...
def print_results_table(records: list[dict[str, object]], title: str) -> None:
    ...
    if not records:
        print("(no records)")
        return
    header = f"{'method':<8} {'variable':<12} {'exact':>8} {'select/cal':>10} {'site/config':<32}"
    print(header)
    print("-" * len(header))
    for record in records:
        ...
        print(
            ...
            f"{str(record.get('variable', 'average')):<12} "
