"""DAS search over RAVEL residual-stream sites."""
...
from .data import RAVELPairBank, RAVELPairDataset
from mcqa_experiment.intervention import DASSubspaceIntervention, run_das_residual_intervention
from .metrics import (
...
class DASConfig:
    """Hyperparameters controlling RAVEL DAS search."""
...
def train_das_candidate(
    *,
    ...
    loader = DataLoader(RAVELPairDataset(bank), batch_size=batch_size, shuffle=True)
    losses: list[float] = []
    previous_loss = None
    plateau_steps = 0
    for epoch_index in range(int(max_epochs)):
        ...
        for batch in loader:
            ...
            loss = cross_entropy_for_das(logits, batch["answer_token_id"].view(-1))
        ...
        if verbose:
            print(
                ...
                f"variable={bank.target_var} source_type={bank.source_type} "
                f"site={site.label} dim={int(subspace_dim)} loss={float(epoch_loss):.6f}"
            )
...
def evaluate_das_candidate(
    *,
    ...
    loader = DataLoader(RAVELPairDataset(bank), batch_size=batch_size, shuffle=False)
    logits_all = []
    for batch in loader:
        logits = run_das_residual_intervention(
            model=model,
...
def run_das_pipeline(
    *,
    model,
    train_bank: RAVELPairBank,
    calibration_bank: RAVELPairBank,
    holdout_bank: RAVELPairBank,
    ...
    if config.verbose:
        print(
            f"[DAS] start variable={train_bank.target_var} source_type={train_bank.source_type} "
        ...
        for subspace_dim in subspace_dims:
            ...
            if config.verbose:
                print(
                    ...
                    f"variable={train_bank.target_var} source_type={train_bank.source_type} "
            ...
            if holdout_metrics_for_candidate is not None:
                ...
                if "changed_exact_acc" in holdout_metrics_for_candidate:
                    record["holdout_changed_exact_acc"] = float(holdout_metrics_for_candidate["changed_exact_acc"])
                if "invariant_exact_acc" in holdout_metrics_for_candidate:
                    record["holdout_invariant_exact_acc"] = float(holdout_metrics_for_candidate["invariant_exact_acc"])
            search_records.append(record)
            if config.verbose:
                message = (
                    f"[DAS] calibration variable={train_bank.target_var} source_type={train_bank.source_type} "
                    f"site={site.label} dim={int(subspace_dim)} epochs={len(loss_history)} "
            ...
            if best is None or float(record["selection_exact_acc"]) > float(best["selection_exact_acc"]):
                ...
                if config.verbose:
                    print(
                        f"[DAS] new best variable={train_bank.target_var} source_type={train_bank.source_type} "
                        f"site={site.label} dim={int(subspace_dim)} "
    ...
    if config.verbose:
        print(
            f"[DAS] holdout variable={train_bank.target_var} source_type={train_bank.source_type} "
