"""Shared comparison runner for RAVEL OT/UOT/DAS experiments."""
...
class CompareExperimentConfig:
    """Config controlling one end-to-end RAVEL comparison run."""
    ...
    target_vars: tuple[str, ...] = ("Continent", "Country", "Language")
    batch_size: int = 16
    ot_epsilon: float = 1.0
    uot_beta_abstract: float = 1.0
    uot_beta_neural: float = 1.0
    signature_mode: str = "answer_token_delta"
...
def run_comparison(
    *,
    ...
    for method in config.methods:
        print(
            f"[method] start method={method} source_type={config.source_type} "
            f"targets={list(config.target_vars)}"
        )
        for target_var in config.target_vars:
            start = perf_counter()
            print(f"[method] method={method} source_type={config.source_type} target={target_var}")
            ...
            if method in {"ot", "uot"}:
                ...
                if prepared_ot_artifacts_by_target is not None:
                    prepared_artifacts = prepared_ot_artifacts_by_target.get(target_var)
            ...
            if method in {"ot", "uot"} and prepared_ot_artifacts_by_target is not None:
                prepared_artifact = prepared_ot_artifacts_by_target.get(target_var)
                if prepared_artifact is not None:
                    signature_prepare_runtime_seconds = float(prepared_artifact.get("prepare_runtime_seconds", 0.0))
            ...
            print(
                f"[method] done method={method} source_type={config.source_type} target={target_var} "
    ...
    print_results_table(all_records, f"RAVEL Counterfactual Test Results [{config.source_type}]")
    print_results_table(summary_records, f"RAVEL Method Average Summary [{config.source_type}]")
